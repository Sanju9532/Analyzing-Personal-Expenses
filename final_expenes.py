...
import pandas as pd
df = pd.read_csv("C:\\Users\\megal\\Downloads\\sanju guvi\\new monthly expenses-1.csv")
...
if selected_option == 'Min Amount':
    cursor.execute("SELECT MIN(Amount_Paid) FROM analyzing_personal_expenses")
    Minimum_expenses = cursor.fetchall()
    st.write(f"Minimum Expenses: {Minimum_expenses}")

if selected_option == 'Max Amount':
    cursor.execute("SELECT MAX(Amount_Paid) FROM analyzing_personal_expenses WHERE Amount_Paid IS NOT NULL")
...
if selected_option == 'CA':
    cursor.execute("SELECT Cashback,Amount_Paid FROM analyzing_personal_expenses ")
...
if opt == "pie":
    ...
    st.subheader('Paymentif Mode Distribution')
    st.write(fig)
...
st.header("Top Categories With Months")
